...
Postman.prototype.setExchange = function (name, type, options = {}) {
    return this.exchange = Exchange(name, type, options);
}
    ...
    this.connection.initialize()
        ...
        .then(ch => {
            if (this.exchange instanceof Exchange) {
                ...
                return ch.assertQueue(this.exchange.routingKey, {
                        exclusive: true
                    }).then((q) => ch.bindQueue(q.queue, this.exchange.name, this.exchange.routingKey))
                    .then(() => ch.consume(this.exchange.routingKey, (msg) => {
                        callback(msg);
